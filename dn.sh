...
dn_install() { ## TODO: testing aliasing!
  ...
  case "${SHELL}" in
    */zsh)
      script_path="${0}"
      local rc_file="${HOME}/.bashrc" # this is incorrect for zsh!!
      echo "###GENERATED BY DN.SH--DO NOT MODIFY" >> ${rc_file}
      echo "source ${DN_INSTALL_DIR}/aliases.sh" >> ${rc_file}
      # ^^ or something -- zsh has a custom.sh where you can add your stuff
  ...
  # save current script version in an easy to access file for now
  # printf "${DN_SCRIPT_VERSION}" > ${DN_INSTALL_DIR}/.version;
  # TODO: ^ if I get installed version by by grepping dn.sh then I don't need this line
  ...
  # # switch global version to default
  # dn_switch -g $DN_INSTALL_NODE_VERSION
  # TODO: ^why not do this with dn_add_and_switch
  dn_add_and_switch -g $DN_INSTALL_NODE_VERSION
  # TODO: this should ork -- verify
...
dn_uninstall() { ## TODO: testing aliasing!!
  ...
  rm -f ${DN_BIN_DIR}/dn;
  ### removes aliases -- todo zsh
  local rc_file="${HOME}/.bashrc"
  sed '/###GENERATED BY DN.SH--DO NOT MODIFY>>>>>/{N;d;}' ${rc_file}
