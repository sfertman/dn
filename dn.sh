...
dn_on() {
  # _alias() { for x in $@; do alias $x="dn run $x"; done; }
  # _alias node npm npx yarn yarnpkg;
  alias node='dn run node';
  alias npm='dn run npm';
  alias npx='dn run npx';
  alias yarn='dn run yarn';
  alias yarnpkg='dn run yarnpkg';
}


dn_off() {
  _unalias() { for x in $@; do unalias "$x"; done; }
  _unalias node npm npx yarn yarnpkg;
}

aliases() { for x in $@; do alias $x="dn run $x"; done }
...
dn_install() {
  ...
  case "${SHELL}" in
    */zsh)
      ...
      echo "${DN_INSTALL_DIR}/dn.sh on" >> ${rc_file}
      ;;
    */bash|*/sh)
      ...
      ## TODO: save enaled/disabled flag and run dn on only if enabled=1
      ;;
    *)
...
#   cat > "${DN_INSTALL_DIR}/alias.sh" <<EOF
# alias ${prefix}node='dn run node' \
# ${prefix}npm='dn run npm' \
# ${prefix}npx='dn run npx' \
# ${prefix}yarn='dn run yarn' \
# ${prefix}yarnpkg='dn run yarnpkg'
# EOF

#   cat <<EOF > "${DN_INSTALL_DIR}/unalias.sh"
# unalias ${prefix}node \
# ${prefix}npm \
# ${prefix}npx \
# ${prefix}yarn \
